...
export function ClientLogoMarquee({ logos, className }: ClientLogoMarqueeProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex animate-gd-client-scroll items-center whitespace-nowrap py-5 group-hover:[animation-play-state:paused] motion-reduce:animate-none motion-reduce:flex-wrap motion-reduce:justify-center motion-reduce:gap-6 motion-reduce:whitespace-normal motion-reduce:py-8">
        {loop.map((logo, index) => (
          <div
            key={`${logo.id}-${index}`}
